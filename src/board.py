...
class Board:
    ...
    def __init__(self):
        self.pixel_rows = Settings.read_variable('System', 'pixel_rows', Importance.CRITICAL)
        ...
        self.pixel_cols = Settings.read_variable('System', 'pixel_cols', Importance.CRITICAL)
        if self.pixel_cols % self.SCREEN_RATIO != 0:
            logging.error("[System] pixel_cols must be a multiple of 16 to work with the 'rpi-rgb-led-matrix' library.")
            logging.error("[System] Exiting program.")
            sys.exit()

        self.encoder_A = Settings.read_variable('Pinout', 'encoder_a', Importance.CRITICAL)
        self.encoder_B = Settings.read_variable('Pinout', 'encoder_b', Importance.CRITICAL)
        self.encoder_button = Settings.read_variable('Pinout', 'encoder_button', Importance.CRITICAL)
        self.tilt_switch = Settings.read_variable('Pinout', 'tilt_switch', Importance.CRITICAL)
        ...
        self.encoder_button.when_pressed = lambda button: self.encoder_button_function(button, self.input_status_dictionary)

        self.encoderQueue = queue.Queue()
        self.encoder = RotaryEncoder(self.encoder_A, self.encoder_B)
        self.encoder.when_rotated_clockwise = lambda enc: self.rotate_clockwise(enc, self.encoderQueue)
        self.encoder.when_rotated_counter_clockwise = lambda enc: self.rotate_counter_clockwise(enc, self.encoderQueue)
        self.encoder_state = 0

        self.tilt_switch = Button(self.tilt_switch, pull_up=True)
        self.is_horizontal_dictionary = {"value": True}
        self.tilt_switch.when_pressed = lambda button: self.tilt_callback(button, self.is_horizontal_dictionary)
        self.tilt_switch.when_released = lambda button: self.tilt_callback(button, self.is_horizontal_dictionary)
    ...
    def encoder_button_function(self, enc_button, inputStatusDict):
        ...
        if time_diff >= hold_time:
            ...
            enc_button.when_pressed = lambda button: self.encoder_button_function(button, inputStatusDict)
            return